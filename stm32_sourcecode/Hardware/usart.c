...
extern unsigned int StepCount,Distance; 
static unsigned char i;
unsigned int Uart2_Len;
extern unsigned int Uart2_Sta;
...
void uart1_init(u32 bound)
{
    ...
    USART_ITConfig(USART1, USART_IT_RXNE, ENABLE);//关闭串口接受中断
...
{ 
     ...
     { 
        ...
        USART2_RX_BUF[Uart2_Rx] = USART_ReceiveData(USART2); 
        Uart2_Rx++; 
        Uart2_Rx &= 0xFF; 
     }                                   
     if(USART2_RX_BUF[Uart2_Rx-1] == 0x5A) //头 
        Uart2_Tx = Uart2_Rx-1; 
     if((USART2_RX_BUF[Uart2_Tx] == 0x5A)&&(USART2_RX_BUF[Uart2_Rx-1] == 0xA5)) //检测到头的情况下检测到尾 
     { 
