...
#else
/* #define SYSCLK_FREQ_HSE    HSE_VALUE */	//8M
/* #define SYSCLK_FREQ_24MHz  24000000 */ 
...
static void SetSysClockTo72(void)
{
  ...
  /* SYSCLK, HCLK, PCLK2 and PCLK1 configuration ---------------------------*/    
  /* Enable HSE */  //使能HSE，需要等待晶振稳定		RCC_CR_HSEON=0x00010000(修改的是第16位HSEON)  
  RCC->CR |= ((uint32_t)RCC_CR_HSEON);		
  ...
  {
    /* Enable Prefetch Buffer */	//使能 FLASH 预存取缓冲区
    FLASH->ACR |= FLASH_ACR_PRFTBE;
    ...
    //配置AHB预分频器分频系数为1,使HCLK = SYSCLK=72M
    RCC->CFGR |= (uint32_t)RCC_CFGR_HPRE_DIV1;	      
    //配置PCLK2预分频器分频系数为1,使PCLK2 = HCLK=72M
    RCC->CFGR |= (uint32_t)RCC_CFGR_PPRE2_DIV1;    
		//设置PCLK1预分频器分频系数为2,使PCLK1 = HCLK/2=36M
    RCC->CFGR |= (uint32_t)RCC_CFGR_PPRE1_DIV2;
		
		
		//设置 PLL 时钟来源，设置 PLL 倍频因子
    /*  PLL configuration: PLLCLK = HSE * 9 = 72 MHz */
    RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_PLLSRC | RCC_CFGR_PLLXTPRE |RCC_CFGR_PLLMULL));
    RCC->CFGR |= (uint32_t)(RCC_CFGR_PLLSRC_HSE | RCC_CFGR_PLLMULL9);

    /* Enable PLL *///使能PLL时钟,RCC->CR寄存器第24位置1	
    RCC->CR |= RCC_CR_PLLON;	//RCC_CR_PLLON=0x01000000

    /* Wait till PLL is ready */	//等待PLL时钟源就绪  
    while((RCC->CR & RCC_CR_PLLRDY) == 0)	//RCC_CR_PLLRDYRCC->CR寄存器第25位  
    {
		}
    
    /* Select PLL as system clock source */	//选择PLL作为系统时钟来源
    RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_SW));	//RCC_CFGR_SW_PLL=0x00000002(RCC->CFGR第1,0位设置为10,见手册)
    RCC->CFGR |= (uint32_t)RCC_CFGR_SW_PLL;    
